...
class BofACashProWebhookHandler
{
    ...
    public function __construct(
        ...
        // FIX-5 (BUG5/SEC4): String vazia passa o isset()/!== null mas produz HMAC
        // criptograficamente inútil (chave = ''), o que é equivalente a sem validação.
        // Rejeitar explicitamente para evitar falsa sensação de segurança.
        if ($this->webhookSecret !== null && $this->webhookSecret === '') {
            throw new \InvalidArgumentException(
                'BofA WebhookHandler: webhookSecret cannot be an empty string. ' .
                'Use null to explicitly disable signature validation (sandbox only).'
            );
        }
    }
